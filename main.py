...
btn_export = QPushButton('Xuất dữ liệu')
...
data_buffer = []
all_data = []  # ✅ Bộ đệm lưu tất cả dữ liệu
is_paused = False
        ...
        if not is_paused:
            if ser.in_waiting > 1:
                ...
                if len(data_buffer) >= BUFFER_LENGTH:
                    ...
                    all_data.extend(data_buffer)  # ✅ Lưu vào bộ đệm tổng
                    data_buffer = []
        ...
        if all_data:
            ...
            with open(file_path, 'w') as f:
                for value in all_data:
                    f.write(f"{value}\n")
            print("✅ Dữ liệu đầy đủ đã được xuất ra file 'serial_data.txt'.")
        else:
...
def export_last_10s():
    ...
    try:
        if all_data:
            ...
            export_data = all_data[-NUM_SAMPLES:] if len(all_data) > NUM_SAMPLES else all_data

            current_dir = os.path.dirname(os.path.abspath(__file__))
            file_path = os.path.join(current_dir, 'last_10s_data.txt')
            with open(file_path, 'w') as f:
                for value in export_data:
                    f.write(f"{value}\n")
            print("✅ Dữ liệu 10 giây gần nhất đã được lưu vào 'last_10s_data.txt'.")
        else:
            print("⚠ Không có dữ liệu để lưu.")
    except Exception as e:
        print(f"❌ Lỗi khi lưu dữ liệu 10 giây: {e}")

# Kết nối các nút
btn_zoom_in.clicked.connect(zoom_in)
btn_zoom_out.clicked.connect(zoom_out)
btn_pause.clicked.connect(pause_update)
btn_export.clicked.connect(export_last_10s)
